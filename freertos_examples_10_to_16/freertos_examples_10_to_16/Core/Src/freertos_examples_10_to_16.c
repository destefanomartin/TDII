...
#define EXAMPLE_17 (17)
#define TEST (EXAMPLE_12)
...
int main_(void)
{
	...
	if (xQueue != (xQueueHandle)NULL) {
		 ...
		 * priority 1. */
		xTaskCreate(vSenderTask, (const char *) "vSender1", (2 * configMINIMAL_STACK_SIZE), (void *) 100,
					(tskIDLE_PRIORITY + 1UL), (xTaskHandle *) NULL);
		xTaskCreate(vSenderTask, (const char *) "vSender2", (2 * configMINIMAL_STACK_SIZE), (void *) 200,
					(tskIDLE_PRIORITY + 1UL), (xTaskHandle *) NULL);

		/* Create the task that will read from the queue.  The task is created with
		 * priority 2, so above the priority of the sender tasks. */
		xTaskCreate(vReceiverTask, (const char *) "vReceiver", (2 * configMINIMAL_STACK_SIZE), NULL,
...
int main_(void)
{
	...
	if (xQueue != (xQueueHandle)NULL) {
		...
		xTaskCreate(vSenderTask, (const char *) "vSender1", (2 * configMINIMAL_STACK_SIZE), (void *) &(xStructsToSend[0]),
					(tskIDLE_PRIORITY + 2UL), (xTaskHandle *) NULL);
		xTaskCreate(vSenderTask, (const char *) "vSender2", (2 * configMINIMAL_STACK_SIZE), (void *) &(xStructsToSend[1]),
					(tskIDLE_PRIORITY + 2UL), (xTaskHandle *) NULL);
 ...
 * interrupt is used, which happens to be the watchdog on the STM32F401RE.  WWDG_IRQHandler */
...
int main_(void)
{
    ...
    if (xBinarySemaphore != (xSemaphoreHandle) NULL) {
        ...
        xTaskCreate(vHandlerTask, (const char *) "Handler", (2 * configMINIMAL_STACK_SIZE), NULL,
        			(tskIDLE_PRIORITY + 3UL), (xTaskHandle *) NULL);

        /* Create the task that will periodically generate a software interrupt.
         * This is created with a priority below the handler task to ensure it will
         * get preempted each time the handler task exits the Blocked state. */
        xTaskCreate(vPeriodicTask, (const char *) "Periodic", (2 * configMINIMAL_STACK_SIZE), NULL,
        			(tskIDLE_PRIORITY + 1UL), (xTaskHandle *) NULL);
...
int main_(void)
{
	...
	routine.  The task is created at priority 1. */
   	xTaskCreate(vIntegerGenerator, (const char *) "IntGen", (2 * configMINIMAL_STACK_SIZE), NULL,
   				(tskIDLE_PRIORITY + 1UL), (xTaskHandle *) NULL);

    /* Create the task that prints out the strings sent to it from the interrupt
     * service routine.  This task is created at the higher priority of 2. */
    xTaskCreate(vStringPrinter, (const char *) "String", (2 * configMINIMAL_STACK_SIZE), NULL,
        			(tskIDLE_PRIORITY + 2UL), (xTaskHandle *) NULL);
...
int main_(void)
{
	...
	if (xMutex != NULL) {
		...
		xTaskCreate(prvPrintTask, (const char *) "Print1", (2 * configMINIMAL_STACK_SIZE),
					"Task 1 ******************************************\r\n",
					(tskIDLE_PRIORITY + 1UL), (xTaskHandle *) NULL);
		xTaskCreate(prvPrintTask, (const char *) "Print2", (2 * configMINIMAL_STACK_SIZE),
					"Task 2 ------------------------------------------\r\n",
					(tskIDLE_PRIORITY + 2UL), (xTaskHandle *) NULL);
...
int main_(void)
{
	...
	if (xPrintQueue != NULL) {
		...
		xTaskCreate(prvStdioGatekeeperTask, (const char *) "Gatekeeper", (2 * configMINIMAL_STACK_SIZE),
...
#if (TEST == EXAMPLE_17)		/* Using a binary semaphore to synchronize a task with an interrupt */
